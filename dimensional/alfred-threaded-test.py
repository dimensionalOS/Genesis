...
class GenesisRosNode(Node):
    def __init__(self, joint_names):
        ...
        self.alfred_tf_subscription = self.create_subscription(
            Twist, 
            '/alfred_base_center_tf_twist', 
            self.base_twist_callback, 
            10
        )
    ...
    def get_base_twist(self):
        with self.twist_lock:
            # Return a copy to avoid threading issues
            return copy.deepcopy(self.base_twist)
...
class RosThread(threading.Thread):
    ...
    def run(self):
        ...
        self.node = GenesisRosNode(self.joint_names)
    ...
    def get_base_twist(self):
        if self.node is not None:
            return self.node.get_base_twist()
        return Twist()
...
def main():
    ...
    ros_thread = RosThread(joint_names)
    ros_thread.start()
    
    # Small delay to allow ROS thread to initialize
    time.sleep(0.5)
    ...
    try:
        ...
        for step in range(10000):
            # Get the current joint positions and base twist from the ROS thread
            joint_positions = ros_thread.get_joint_positions()
            base_twist = ros_thread.get_base_twist()
            
            if step < 100:
                target_positions = np.array([joint_positions[name] for name in valid_joint_names])
            ...
            robot.set_pos([base_twist.linear.x, base_twist.linear.y, base_twist.linear.z])
            # Create quat from euler from base_twist.angular
            quat = R.from_euler('zyx', [base_twist.angular.x, base_twist.angular.y, -base_twist.angular.z + 90], degrees=True).as_quat()
            robot.set_quat(quat)
